...
export default function Dashboard() {
  const { transactions, products, currentUser } = useAppContext();
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 flex items-center space-x-4">
          ...
          <div>
            ...
            <h3 className="text-2xl font-bold text-slate-800">{todayTransactions.length} Transaksi</h3>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 flex items-center space-x-4">
          ...
          <div>
            ...
            <h3 className="text-2xl font-bold text-slate-800">{totalItemsSoldToday} Item</h3>
          </div>
        </div>
      </div>
               ...
               {lowStockProducts.map(p => (
                 <li key={p.id} className="flex items-center justify-between pb-3 border-b border-slate-100 last:border-0 last:pb-0">
                    ...
                    {currentUser?.role === 'Owner' && (
                       <button className="text-xs text-indigo-600 hover:text-indigo-800 font-medium">Update</button>
                    )}
                 </li>
               ))}
            </ul>
